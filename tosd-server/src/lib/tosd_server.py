...
class TOSDService:
    ...
    <node>
        <interface name='org.theom.tosd'>
            <method name='ShowOSD'>
                ...
                <arg type='i' name='margin_x' direction='in'/>
                <arg type='i' name='margin_y' direction='in'/>
    ...
    def ShowOSD(self, text, mode, value, duration, size, position, margin_x, margin_y, 
                dont_reuse_window, background, text_color, slider_fill_color, slider_knob_color):

        if not dont_reuse_window: # if dont reuse window is not called, reuse the window
            reuse_osd_window()

        def gtk_callback():
            show_osd(text, mode, value, duration, size, position, margin_x, margin_y,
